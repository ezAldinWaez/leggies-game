# Index

This document will have information on the scripts we wrote. For each script, we will describe why we wrote it, what it does, and how to use it (regardless of whether you read the code).

After each script is the date of documentation. We will document each script after finishing it, hopefully.

## Contents

### Static Scripts

Scripts that will not be used as components, used mostly to organize stuff then components will use them.

- [[InputName]]
- [[DefaultInputDictionaries]]
- [[LeggiesLibrary]]
___

### Map Scripts

- [[PlayerBuilder]]
- [[PlayerPositioner]]
- [[PlayerInputSetter]]
- [[PlayerListManager]]
___

### Player Scripts

We will discuss here the scripts that affect the player (whether a Leggie or something else).

- [[PlayerInput]]
- [[Move]]
- [[Retreat]]
- [[Attack]]
- [[Feint]]
- [[DetectAttackables]]
___

### Main Camera Scripts

We will discuss here scripts that deal with the camera, mainly its positioning.

- [[CameraFollow]]
___

### Misc Scripts

- [[SpeedLimiter]]
- [[DetectAttack]]
- [[Die]]
- [[FlyAwayAfterDeath]]
___
